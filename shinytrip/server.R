...
library(readr)
library(shinyalert)
restaurant<-data.frame(na.omit(read_csv("restaurant_final.csv")))
...
Theatre<-na.omit(read.csv("Theater.csv",header=T))
  ...
  observe({
    ...
    output$table1 <- renderDataTable({
      ...
      if (input$region1 == 'Museum'){
        ...
        print(Library[,c(1,2,5)])
      }
      else if(input$region1 == 'Restaurant') {
        if (input$type1 == 'ALL'){
          print(restaurant[,c(1,5,9,6)])
        }
        else{
          print(restaurant[restaurant$CUISINE == as.character(input$type1),c(1,5,9,6)])
        }
    ...
    observeEvent(input$submit,{
      if(input$location=="Current Location"){
        ...
        output$map<-renderLeaflet(
          {    map <- leaflet() %>% setView(-73.983,40.7639,zoom = 9) %>% addTiles()
